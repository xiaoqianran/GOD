...
def _call_claude(
    prompt: str, model: str | None, timeout: int = 300
) -> str:
...
def improve_description(
    ...
    """
    failed_triggers = [
        r for r in eval_ctx.results["results"]
    ...
    train_score = (
        f"{eval_ctx.results['summary']['passed']}/"
        f"{eval_ctx.results['summary']['total']}"
    )
    if eval_ctx.test_results:
        test_score = (
            f"{eval_ctx.test_results['summary']['passed']}/"
            f"{eval_ctx.test_results['summary']['total']}"
        )
